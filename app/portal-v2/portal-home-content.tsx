...
import { loadViewerBootstrap, type ViewerBootstrap } from "@/lib/viewer-scope";

import PortalV2Client from "./portal-v2-client";

export default async function PortalHomeContent() {
  let bootstrap: ViewerBootstrap | null = null;
  try {
    bootstrap = await loadViewerBootstrap();
  } catch (err) {
    console.error("[portal-v2] bootstrap failed", err);
    bootstrap = null;
  }
